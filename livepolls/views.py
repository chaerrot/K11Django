...
단축함수: 장고에서 웹 프로그램 개발 시 자주 사용하는 기능들을
    내장 함수로 제공하고 있는데 이를 단축 함수라 표현한다.
    render() 함수 등이 있다.
'''
...
def main(request): # views에 함수 정의 시 request 내장 객체는 필수 요소임
    return render(request, 'livepolls/main.html')

# 투표 앱의 첫 번째 화면으로 질문이 리스팅된다.
def index(request):
...
def detail(request, question_id):
    # URL패턴을 통해 전달된 일련번호를로 Question테이블을 검색 후 반환한다.
...
def vote(request, question_id):
    ...
    try:
        ...
        # 예외 발생이 되면 detail페이지를 다시 보여준다. 이 때 에러메세지를 전달한다.
        return render(request, 'livepolls/detail.html', {
            'question':question,
            'error_message':"You didn't select a choice",
        })
    else:
        # try절의 코드가 정상실행되면 가져온 레코드를 통해 vote항목을 +1 업데이트한다.
        selected_choice.votes += 1
        # 앞에서 처리한 내용을 실제 테이블에 적용한다.
        selected_choice.save()
        # 정상 처리되면 reuslt 페이지로 이동한다.

...
class QuestionForm(forms.Form):
    ...
    '''
        추가적인 속성을 부여할 때는 위젯에 attrs을 기술한다.
    '''
    ...
    select 태그를 표현한다. (1개 선택)
        ...
        initial: 해당 태그의 default 값으로 사용한다. 주로 수정에서 
            활용하면 된다.
    ...
    또한 default 값을 표현하기 위해 2개 이상의 항목을 선택할 수 있어야
    하므로 리스트를 사용해야 한다.
    '''
    ...
    <div>
        <label for="id_form7_0">
            <input type="checkbox" name="form7" value="red" id="id_form7_0">빨강
        </label>
    </div>
    <div>
    <label for="id_form7_1">
        <input type="checkbox" name="form7" value="green" id="id_form7_1" checked>초록
    </label>
    </div>
    </div>
    라디오 버튼을 표현한다. (1개 선택)
    ...
    '''
    체크박스를 표현한다. (멀티 선택)
    추가적인 속성을 부여할 때는 attrs={'class':'red'} 와 같이 기술한다.
    '''
